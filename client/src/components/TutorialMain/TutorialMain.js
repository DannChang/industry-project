...
export default function TutorialMain(props) {
  console.log(props);
  const { headerText, upperText, lowerText, prevLink, nextLink } =
    props.cardInfo;
        ...
        <section className={"modal modal" + props.page}>
          <article className="modal__card">
            <section className="modal__header">
              ...
              <h1 className="modal__header-text">{headerText}</h1>
            </section>
            <section className="modal__text">
              <p className="modal__upper-text">{upperText}</p>
              <p className="modal__lower-text">{lowerText}</p>
            </section>
            <section className="modal__footer">
              {prevLink && (
                <Link to={`/tutorial/${prevLink}`}>
                  <img
                    className="modal__left-arrow"
                    src={arrowLeft}
                    alt="Previous page"
                  />
                </Link>
              ...
              {!prevLink && (
                <div>
                  <img
                    className="modal__left-arrow modal__left-arrow--unavailable"
                    src={arrowLeftUnable}
              ...
              {!nextLink && (
                <div>
                  <img
                    className="modal__right-arrow modal__right-arrow--unavailable"
                    src={arrowRightUnable}
                    alt="Next page unavailable"
                  />
                </div>
              )}

              <a href={`http://localhost:5500/server/index.html`}>
                <div className="modal__skip">SKIP</div>
              </a>
            </section>
          </article>
          <img className="modal__help" src={help} alt="help button"></img>
        </section>
      )}
    </>
  );
